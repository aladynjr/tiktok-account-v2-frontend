...
const ToolTip = ({ children, tooltip, show, loading }) => {
      ...
      {tooltip ? (
        <span
          ...
          className={clsx("  opacity-0  transition bg-black text-white p-1 rounded absolute top-full mt-2 font-xs whitespace-wrap", show && 'visible opacity-100')}
            style={{fontSize:'13px', backgroundColor:'rgba(0, 0, 0, .8)', width:'216px', transform: 'translateX(-50%)', marginLeft: (loading) ? '-10px' : '-46px'}}
